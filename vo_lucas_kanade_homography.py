...
def vo_KLT():
    ...
    for i in range(1, len(dl.image_files)-1):
        ...
        # ---------------- Rotation aus Homographie ----------------
        R_img = H[0:2, 0:2]

        # Orthonormalisieren (sehr wichtig!)
        U, _, Vt = np.linalg.svd(R_img)
        R_img = U @ Vt

        theta_img = np.arctan2(R_img[1,0], R_img[0,0])
        yaw = -theta_img  # Bild → Welt
        ...
        # Skalenkompensation (Homographie!)
        scale = np.linalg.norm(R_img[:,0])
        if scale > 1e-6:
            t_img /= scale

        # Translation in Welt drehen
        t_world = R_kitti[:2, :2] @ t_img
