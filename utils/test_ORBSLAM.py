...
def calc_scale(gt_file="gt.txt", est_file="est.txt"):
    ...
    N = min(len(gt_data), len(est_data))
    gt = gt_data[:N]
    est = est_data[:N]
    ...
    with open("test_ORBSLAM_results.txt", "w") as f:
    ...
    #ax.set_xlabel('[m]')
    #ax.set_ylabel('[m]')
    ax.set_title('Trjektorie GT vs. ORB-SLAM-3 Estimated')
    ax.grid(True)
    ax.axis('equal')
    ax.legend()
    plt.savefig("test_ORBSLAM.png")

    plt.show()
    plt.close()


calc_scale("/home/tore/Volume/1000x1000_droidtest3/gt.txt", "/home/tore/Volume/1000x1000_droidtest3/ORBSLAM_GESTrajectoryKITTI_2.txt")
