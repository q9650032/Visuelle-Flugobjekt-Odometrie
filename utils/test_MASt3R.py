...
def calc_scale(gt_file="gt.txt", est_file="est.txt"):
    ...
    plt.axis("equal")
    plt.legend()
    plt.title("TEST MASt3R")
    plt.savefig("test_MASt3R.png")
    ...
    with open("test_MASt3R_results.txt", "w") as f:
    ...
    #ax.set_xlabel('[m]')
    #ax.set_ylabel('[m]')
    ax.set_title('Trjektorie GT vs. MASt3R Estimated')
    ax.grid(True)
    ax.axis('equal')
    ax.legend()
    plt.savefig("test_MASt3R.png")
...
calc_scale("/home/tore/Volume/640x480_norot_noh/gt.txt", "/home/tore/Volume/github/MASt3R-SLAM/logs/1000x1000_droidtest3/footage.txt")
